...
class TestCategoriesModel(TestCase):
    ...
    def setUp(self):
        # create data that we want to test against/replica of real data
        self.data1 = Category.objects.create(name='django', slug='django')

    def test_category_model_entry(self):  # testing catagory insertion
        """
        Test Category model data insertion/types/field attributes
        """
        data = self.data1
        # test data against model. If datatypes match, return true
        self.assertTrue(isinstance(data, Category))
        # check weather the default names match
